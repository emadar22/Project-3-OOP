...
OOP_Hw3::MultiplyFunction::MultiplyFunction(const SharedPtr<OOP_Hw3::Function> f1,
...
double OOP_Hw3::MultiplyFunction::Evaluate(const map<string, double> &variables) const {
    return (func1->Evaluate(variables))*(func2->Evaluate(variables));
}
SharedPtr<OOP_Hw3::Function> OOP_Hw3::MultiplyFunction::DeriveBy(const string &variable) const {
    ...
    SharedPtr<Function> res= new AddFunction(first,second);
    return res;
}
map<string,map<string,SharedPtr<OOP_Hw3::Function> > > OOP_Hw3::MultiplyFunction::Hessian() const {
